...
export default function Header({ onSearchChange }: Props) {
  ...
  useEffect(() => {
    setDropdownVisible(isCategoryPage);
  }, [isCategoryPage]);
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    onSearchChange(value); // Optional: still send to parent if needed
  };
  ...
  return (
    <header className="bg-white border-b border-gray-200 text-sm">
      <div className="px-4 py-2 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
        {/* Logo */}
        <div className="text-xl md:text-[28px] font-extrabold tracking-wide text-black">SHEIN</div>
      ...
      <div className="hidden md:flex w-full">
        <nav className="flex items-center gap-5 px-6 py-1 text-[13px] font-medium text-gray-800">
          {/* Dropdown menu */}
          <div
            className="relative"
              ...
              <div className="absolute top-6 left-0 bg-white shadow-lg rounded w-64 max-h-[400px] overflow-y-auto z-50 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                <ul className="divide-y divide-gray-100">
                  {categories.map((cat, i) => (
                    <li
                      key={i}
                      className="flex items-center justify-between px-4 py-2 text-[14px] text-gray-800 hover:bg-gray-100 cursor-pointer"
                    >
                      <Link
      ...
      <div className="md:hidden flex items-center gap-4 px-3 py-2 overflow-x-auto whitespace-nowrap scrollbar-hide text-[13px] font-medium text-gray-800">
        <div
